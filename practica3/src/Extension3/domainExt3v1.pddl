(define (domain VidaEnMarte_Extension1)
    ...
    (:action descargar_Personal
        ...
        :effect (and (Servida ?peticion) (not (Dentro ?peticion ?rover)) (decrease (personalEnRover ?rover) 1) (decrease (recursosDisponibles) 1) (increase (prioridadTotal) (prioridad ?peticion)))
    )
    ...
    (:action descargar_Suministros 
        ...
        :precondition (and (Dentro ?peticion ?rover) (not (Servida ?peticion)) (Estacionado ?rover ?asentamiento) (ContenidoPeticion ?peticion ?suministro) 
                    (DestinoPeticion ?peticion ?asentamiento))
        :effect (and (Servida ?peticion) (not (Dentro ?peticion ?rover)) (decrease (suministrosEnRover ?rover) 1) (decrease (recursosDisponibles) 1) (increase (prioridadTotal) (prioridad ?peticion)))
    )
)